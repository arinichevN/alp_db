function EditProg() {
    ...
    this.init = function () {
        try {
            ...
            this.t1.cellClickControl([true, true, true, true, true, true, true, true, true]);
            this.t1.enable();
            this.saveB = cb("");
            this.getB = cb("");
            ...
            if (this.last_sc === this.t1.sc && this.last_sr === this.t1.sr) {
                switch (this.t1.sc) {
                    case this.ROW.DESCRIPTION:
                        var self = this;
                        vstring_edit_smp.prep(this.data[this.t1.sr].description, app.LINE_SIZE, self, this.t1.sc, 301);
                        showV(vstring_edit_smp);
                        break;
                    case this.ROW.PEER_ID:
                        var self = this;
                        vstring_edit_smp.prep(this.data[this.t1.sr].peer_id, app.NAME_SIZE, self, this.t1.sc, 302);
                        showV(vstring_edit_smp);
                        break;
                    case this.ROW.CHECK_INTERVAL:
                        var self = this;
                        vtime_edit.prep(this.data[this.t1.sr].check_interval, 0, INT32_MAX, self, this.t1.sc, 303);
                        showV(vtime_edit);
                        break;
                    case this.ROW.COPE_DURATION:
                        var self = this;
                        vtime_edit.prep(this.data[this.t1.sr].cope_duration, 0, INT32_MAX, self, this.t1.sc, 304);
                        showV(vtime_edit);
                        break;
                    case this.ROW.PHONE_NUMBER_GROUP_ID:
                        ...
                        vselect_edit.prep(data, false, self, this.t1.sc, 305);
                        showV(vselect_edit);
                        break;
                    case this.ROW.SMS:
                    ...
                    case this.ROW.RING:
                        ...
                        break;
                    case this.ROW.ENABLE:
                        if (this.data[this.t1.sr].enable) {
                            this.data[this.t1.sr].enable = 0;
                            this.t1.updateCell(this.t1.sr, this.t1.sc, "");
                        } else {
                            this.data[this.t1.sr].enable = 1;
                            this.t1.updateCell(this.t1.sr, this.t1.sc, this.ACTIVE_SIGN);
                        }
                        break;
                    case this.ROW.LOAD:
                        if (this.data[this.t1.sr].load) {
                            this.data[this.t1.sr].load = 0;
                            this.t1.updateCell(this.t1.sr, this.t1.sc, "");
                        } else {
                            this.data[this.t1.sr].load = 1;
                            this.t1.updateCell(this.t1.sr, this.t1.sc, this.ACTIVE_SIGN);
                        }
                        break;
                }
        ...
        try {
            switch (k) {
                case this.ROW.DESCRIPTION:
                    ...
                    break;
                case this.ROW.PEER_ID:
                    this.data[this.t1.sr].peer_id = d;
                    this.t1.updateCell(this.t1.sr, this.t1.sc, this.data[this.t1.sr].peer_id)
                    break;
                case this.ROW.CHECK_INTERVAL:
    ...
    this.dataRowChanged = function (i) {
        if (
                ...
                this.data[i].enable !== this.data_ini[i].enable ||
                this.data[i].load !== this.data_ini[i].load
                ) {
            return true;
        }
        return false;
                ...
                case this.ACTION.GET:
                    ...
                    for (i = 0; i < d.length; i++) {
                        this.data.push({
                            ...
                            enable: parseInt(d[i].enable),
                            load: parseInt(d[i].load)
                        });
                        this.data_ini.push({
                            id: parseInt(d[i].id),
                            description: d[i].description,
        ...
        try {
            ...
            for (var i = 0; i < this.data.length; i++) {
                ...
                if (this.data[i].enable) {
                    enable = this.ACTIVE_SIGN;
                }
                var load = "";
                if (this.data[i].load) {
                    load = this.ACTIVE_SIGN;
                }
                this.t1.appendRow([
